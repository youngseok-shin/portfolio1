...
<body>
	...
	<div class="layout">
		...
		<form action="board11.do" method="post">
		...
		<div>
			<table  class="listBoard">
				...
				<c:forEach items="${selectNewsList }" var="news">
					<tr>
						<td>${news.boardNum }</td>
						<td onclick="location.href='board4.do?boardNum=${news.boardNum}'" >
							<label style="cursor:pointer;"> ${news.boardTitle }</label>
						</td>
						<td> ${sessionScope.loginInfo.memberName }</td>
						<td>${news.boardDate }</td>
						<td>${news.boardCnt }</td>
		...
		<div class="last_button">
			<div class="keep_right">
				...
				<c:if test="${sessionScope.loginInfo.memberId eq 'admin' }">
					<button title="글쓰기" class="btn_blue" onclick="location.href='board7.do'; return false;" type="button" >
						<span>글쓰기</span>
					</button>
				</c:if>
